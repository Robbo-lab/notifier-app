# activity/session14/test_integration_example.py
# Linked code: notifier/views/views.py::notify_view and notifier/urls.py::documents_collection
...
# 1. Add these classes to `notifier/tests/test_views_integration.py`.
# 2. Keep the login flow but reuse existing user fixtures if available.
# 3. Ensure the `fake_metadata_fetch` patch points at the production import path.
# 4. Wire any additional fixtures (e.g., documents) using factory helpers once they exist.
# 5. Run `python manage.py test notifier.tests.test_views_integration`.
